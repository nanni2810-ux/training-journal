...
function findExercise(line){for(const [r,c] of CANON)if(r.test(line))return c;return''}
function parsePlannedRows(text){
  ...
  for(const raw of String(text||'').split(/\n+/)){
    ...
    if(!m){
      const one=line.match(/\b(\d+)\s+set(?:s|\b).*?\b(max(?:imo)?|tecnic[oa]|\d+)\b/i);
      if(one){sets=one[1];reps=/max|tecn/i.test(one[2])?'max':one[2]}
      else if(/\b1\s+tentativo\s+max/i.test(line)){sets='1';reps='max'}
    }
    ...
    if(kg)load=kg[1].replace(/\s+/g,' ');else if(pct)load=pct[1].replace(/\s+/g,'');else if(bw)load='BW';
    if(!sets&&!reps&&!load&&!/\b(?:test|max|salire|top|single|triple|complex)\b/i.test(line))continue;
    const notes=line.length>110?line.slice(0,107)+'…':'';
...
function rowHtml(r={}){
  ...
  return `<div class="ae-log-row" data-ae-log-row="${esc(x.id)}">
    ...
    <div class="field"><label>Rep</label><input data-ae-log-field="reps" value="${esc(x.reps)}" placeholder="3 / 1+1 / max"></div>
    <div class="field"><label>Carico</label><input data-ae-log-field="load" value="${esc(x.load)}" placeholder="70 kg / +10 / BW / 85%"></div>
...
function actualSummary(x){
  ...
  const items=rows.slice(0,4).map(r=>{const scheme=[r.sets,r.reps].filter(Boolean).join('×');return `<span class="ae-log-chip"><b>${esc(r.exerciseKey||r.exercise||'Esercizio')}</b><span>${esc(scheme||'')}${r.load?`${scheme?' · ':''}${esc(r.load)}`:''}</span></span>`}).join('');
...
async function openLog(id){
  ...
  let rows=hasSaved?(Array.isArray(old.rows)&&old.rows.length?old.rows:[{}]):parsePlannedRows(planned);
  if(!rows.length)rows=[{}];
  openModal('Registra lavoro svolto',`<form id="aeActualWorkForm" data-id="${esc(id)}">
    ...
    <div class="ae-log-heading-row"><div><h3 class="ae-log-heading">Esercizi realmente svolti</h3><p class="muted tiny">${hasSaved?'Stai modificando il resoconto salvato.':'Precompilati dal lavoro previsto: modifica, elimina o aggiungi ciò che hai realmente fatto.'}</p></div><button type="button" class="button ghost small" data-ae-log-reset>↺ Ripristina dal programmato</button></div>
  ...
  </form>`,'Il lavoro previsto viene riportato automaticamente come base modificabile; il salvataggio conserva solo ciò che dichiari come realmente svolto.');
}

function collectRows(form){return [...form.querySelectorAll('[data-ae-log-row]')].map(row=>{const get=k=>String(row.querySelector(`[data-ae-log-field="${k}"]`)?.value||'').trim();return normalizedRow({id:row.dataset.aeLogRow||uid(),exercise:get('exercise'),sets:get('sets'),reps:get('reps'),load:get('load'),notes:get('notes')})}).filter(r=>r.exercise||r.sets||r.reps||r.load||r.notes)}
async function resetFromPlanned(form){
  const state=await getState(),x=state?.adaptiveExtras?.find(a=>a.id===form.dataset.id);if(!x)return;
  const version=String(new FormData(form).get('version')||'custom');
  const planned=version==='reduced'?x.reducedText:x.mainText;
  if(!planned){alert('Non c’è una versione programmata da riportare.');return}
  const current=collectRows(form);if(current.length&&!confirm('Sostituire le righe attuali con il lavoro programmato?'))return;
  const rows=parsePlannedRows(planned),box=document.getElementById('aeActualRows');if(!box)return;
  box.innerHTML=(rows.length?rows:[{}]).map(rowHtml).join('');
}
async function saveLog(form){
  ...
  x.actualWork={schema:2,version,rows,duration:String(fd.get('duration')||'').trim(),rpe,notes:String(fd.get('notes')||'').trim(),completed,source:'editable_planned_copy',updatedAt:new Date().toISOString(),completedAt:completed?(x.actualWork?.completedAt||new Date().toISOString()):''};
...
async function enhance(){
  ...
  document.querySelectorAll('.ae-card[data-ae-id]').forEach(card=>{
    const id=card.dataset.aeId,x=state.adaptiveExtras.find(a=>a.id===id);if(!x)return;const actions=card.querySelector('.ae-actions');
    if(actions&&!actions.querySelector('[data-ae-log-open]')){const b=document.createElement('button');b.type='button';b.className='button accent small';b.dataset.aeLogOpen=id;b.textContent=x.actualWork?'✎ Modifica svolto':'✓ Registra svolto';actions.appendChild(b)}else if(actions){const b=actions.querySelector('[data-ae-log-open]');if(b)b.textContent=x.actualWork?'✎ Modifica svolto':'✓ Registra svolto'}
    card.querySelector('.ae-log-summary')?.remove();if(x.actualWork){const wrap=document.createElement('div');wrap.innerHTML=actualSummary(x);const node=wrap.firstElementChild;if(node)actions?.insertAdjacentElement('beforebegin',node)}
  });
}
...
document.addEventListener('click',e=>{
  ...
  const reset=e.target.closest('[data-ae-log-reset]');if(reset){e.preventDefault();const form=reset.closest('#aeActualWorkForm');if(form)resetFromPlanned(form);return}
...
let timer=null;new MutationObserver(()=>{clearTimeout(timer);timer=setTimeout(enhance,90)}).observe(APP,{childList:true,subtree:true});
...
.ae-log-heading{font-size:.78rem;margin:15px 0 4px}.ae-log-heading-row{display:flex;align-items:flex-end;justify-content:space-between;gap:10px;margin-bottom:8px}.ae-log-heading-row .ae-log-heading{margin-top:15px}.ae-log-heading-row p{margin:0}.ae-log-row{display:grid;grid-template-columns:minmax(150px,1.55fr) 70px 85px 115px minmax(150px,1fr) 38px;gap:7px;align-items:end;padding:9px;border:1px solid var(--line);border-radius:12px;margin-bottom:7px;background:#fbfcfb}.ae-log-row .field{margin:0}.ae-log-row label{font-size:.62rem}.ae-log-row input{min-width:0}.ae-log-remove{margin-bottom:1px}.ae-log-complete{min-height:42px;display:flex;align-items:center;gap:8px;padding:8px 10px;border:1px solid var(--line);border-radius:10px;font-size:.72rem;font-weight:700}.ae-log-complete input{width:18px;height:18px}
...
@media(max-width:850px){.ae-log-row{grid-template-columns:1.5fr 70px 85px 115px 38px}.ae-log-notes{grid-column:1/-2}}
@media(max-width:600px){.ae-log-heading-row{align-items:flex-start;flex-direction:column}.ae-log-row{grid-template-columns:1fr 1fr}.ae-log-exercise,.ae-log-notes{grid-column:1/-1}.ae-log-remove{grid-column:2;justify-self:end}.ae-log-summary-head{align-items:flex-start;flex-direction:column}}
`;
document.head.appendChild(style);setTimeout(enhance,120);console.info(`Training Journal adaptive work log ${PATCH_VERSION} loaded`);
})();
